...
!function () {
    ...
    if (!window.__skipIntroLoaded) {
        window.__skipIntroLoaded = true;

        // Автообновление плагина в списке
        (function () {
            try {
                ...
                if (Array.isArray(t)) {
                    ...
                    t.forEach(function (plugin) {
                        if (plugin.url && plugin.url.indexOf("lampa-auto-skip") !== -1) {
                            if (plugin.name !== "Skip Intro/Outro") {
                                plugin.name = "Skip Intro/Outro";
                                e = true;
                            }
                            if (plugin.author !== "@vahagn") {
                                plugin.author = "@vahagn";
                                e = true;
                            }
                        }
                    });
                    if (e) Lampa.Storage.set("plugins", JSON.stringify(t));
                }
            } catch (err) { }
        })();
        ...
        var Settings = {
            init: function () {
                ...
                Lampa.SettingsApi.addParam({
                    ...
                    field: { name: "Всегда автопропуск", description: "Всегда перематывать без кнопки (для всех сериалов)" }
                });
        ...
        var SmartSkip = {
            ...
            hasSkipped: function (tmdb, type) {
                return this._getAll()[tmdb + "_" + type] === true;
            },

            rememberSkip: function (tmdb, type) {
                var data = this._getAll();
                data[tmdb + "_" + type] = true;
                this._saveAll(data);
                console.log("[SkipIntro] Smart skip remembered for", tmdb, type);
            },

            forgetSkip: function (tmdb, type) {
                var data = this._getAll();
                delete data[tmdb + "_" + type];
                this._saveAll(data);
                console.log("[SkipIntro] Smart skip forgotten for", tmdb, type);
            }
        };

        // ==================== Локальное хранилище сегментов ====================
        var LocalCache = {
            _key: function (tmdb, season, episode) {
                return "skip_" + tmdb + "_s" + season + "_e" + episode;
            },

            get: function (tmdb, season, episode) {
                try {
                    var item = localStorage.getItem(this._key(tmdb, season, episode));
                    if (!item) return null;
                    var data = JSON.parse(item);
                    if (data._ts && Date.now() - data._ts > 604800000) { // 7 дней
                        localStorage.removeItem(this._key(tmdb, season, episode));
                        return null;
                    }
            ...
            set: function (tmdb, season, episode, segments) {
                try {
                    localStorage.setItem(this._key(tmdb, season, episode), JSON.stringify({
                        segments: segments,
                        _ts: Date.now()
                    }));
                } catch (e) { }
        ...
        var SubtitleDetector = {
            detect: function (player) {
                // ... (полная реализация)
                // (я сократил только здесь для примера, в реальном файле всё будет)
            }
            // ... остальные методы
        };
        ...
        // ==================== UI Кнопка ====================
        var ButtonUI = {
            // ... весь код кнопки, стили, countdown и т.д.
        };
        ...
        var SkipIntro = {
            ...
            init: function () {
                ...
                Lampa.Player.listener.follow("start", function (e) { self._onPlayerStart(e); });
                ...
                console.log("[SkipIntro] Plugin v2.8 initialized | build: 2026");
            },

            _extractMeta: function (data) { /* ... */ },
            _onPlayerStart: function (data) { /* ... */ },
            _runDetection: function (player, meta, callback) { /* ... */ },
            _onTimeUpdate: function (time) { /* ... */ },
            _showNormalButton: function (segment, source) { /* ... */ },
            _showCountdownButton: function (segment, source) { /* ... */ },
